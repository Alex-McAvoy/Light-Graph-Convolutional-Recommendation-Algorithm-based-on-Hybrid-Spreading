...
def recommendForAllUser(model: LightGCN, user_num: int, item_num: int,
...
def recommendLightGCN(user_num:int, item_num: int, 
                      rating_df: pd.DataFrame, train_data_df: pd.DataFrame, 
                      val_data_df: pd.DataFrame, test_data_df: pd.DataFrame) -> dict:
    """
    ...
    logger.info("正在加载模型")
    try:   
        model = torch.load(cfg.MODEL["save_path"] + str(cfg.RECOMMEND["k"]) + "_LightGCN.pth")
        logger.info("模型加载完毕")
    except:
