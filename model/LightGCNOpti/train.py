...
from model.LightGCNOpti.loss import BPRLoss, sampleMiniBatch
from model.LightGCNOpti.evaluation import calValLoss, getValRecommendations
...
def getEmbeddingForBPR(model: LightGCNOpti, user_num: int, item_num: int, 
...
def trainLightGCNOpti(user_num: int, item_num: int, edge_index: torch.Tensor, 
                  train_edge_index: torch.Tensor, val_edge_index: torch.Tensor, 
                  user_features: torch.Tensor, item_features: torch.Tensor) -> LightGCNOpti:
    """
    ...
    model = LightGCNOpti(user_num, item_num, embedding_dim, layers, user_features, item_features)
    ...
    # 保存模型
    torch.save(model, cfg.MODEL["save_path"] + str(k) + "_LightGCNOpti.pth")
    ...
    # 构建DataFrame并保存
    save_path = cfg.PICTURES["save_path"] + "LightGCNOpti_" + str(k) 
