...
from model.LightGCNOpti.recommend import buildGraph
...
def getLightGCNOptiModel(
    ...
    """
    # 构建LightGCN图
    edge_index, train_edge_index, val_edge_index, test_edge_index = buildGraph(
        user_num, item_num, rating_df, train_data_df, val_data_df, test_data_df
    )

    # 用户特征Dataframe转为tensor
    user_features_df_sorted = user_features_df.sort_values(by="user_id")
    user_features_np = (
        user_features_df_sorted["user_features"]
        .apply(lambda row: ast.literal_eval(row) if not isinstance(row, list) else row)
        .tolist()
    )
    ...
    item_features_np = (
        item_features_df_sorted["item_features"]
        .apply(lambda row: ast.literal_eval(row) if not isinstance(row, list) else row)
        .tolist()
    )
    ...
    # 训练
    logger.info("正在加载LightGCNOpti模型")
    try:
        model = torch.load(cfg.MODEL["save_path"] + str(k) + "_LightGCNOpti.pth")
        logger.info("LightGCNOpti模型加载完毕")
    except:
        logger.info("LightGCNOpti模型加载失败，正在重新训练模型")
        model = trainLightGCNOpti(
            user_num,
            item_num,
            edge_index,
            train_edge_index,
...
def getAllocateMat(
    ...
    # 获取LightGCN模型
    model, edge_index, train_edge_index, val_edge_index, test_edge_index = (
        getLightGCNOptiModel(
            user_num,
            item_num,
            rating_df,
            train_data_df,
...
def getHybridSResourceMat(
    A: np.ndarray, general_W: np.ndarray, lambad_val: float
) -> np.ndarray:
    """
...
def getResourceMat(
    ...
    A = getInteractionMatrixByDataframe(
        user_num, item_num, pd.concat([train_data_df, val_data_df])
    )
