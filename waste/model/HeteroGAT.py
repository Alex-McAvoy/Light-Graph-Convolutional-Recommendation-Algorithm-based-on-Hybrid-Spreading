...
def evalHeteroGAT(model: HeteroGAT, graph: HeteroData) -> torch.Tensor:
    """
...
def recommendForAllUser(train_dataset: Dataset, model: HeteroGAT, graph: HeteroData, k: int) -> np.ndarray:
    """
    @description 
    @param {Dataset} train_dataset 内部数据集
    @param {GNN} model HeteroGAT模型
    @param {Data} graph HeteroGAT图
    @param {int} k 推荐列表大小
    @return {np} 获取所有用户的推荐列表
    """

    # 用户数
    user_num = train_dataset.uIdx_np.shape[0]
    # 所有用户的推荐列表
    recommendation_np = evalHeteroGAT(model, graph)
...
def recommendHeteroGAT(train_dataset: Dataset, k: int = cfg.RECOMMEND["k"]) -> dict:
    """
